...
|Total number of tweets **by** `r params$client`      |`r nrow(tweets_by_us)`                            |
|Total number of tweets **on** `r params$client`      |`r nrow(wk_data)`                                 |
...
```{r, message=FALSE}
                        ...
                        # Facebook #
                        ############
## Load data on Facebook Page posts
...
mth_Posts <- filter(fbPosts, created_mth == thisMth)
...
|                                           |                              |
|-------------------------------------------|------------------------------|
|`r params$client` Page Posts (All-time)    |    `r nrow(fbPosts)`         |
|Posts in `r thisMth`                       |`r nrow(mth_Posts)`|
|Posts in the past 7 days                  |`r nrow(wk_Posts)`          |
|Most Liked  (Overall) |`r message[match(max(likes_count), likes_count)]`|
|Most Liked Post in `r thisMth` |`r # no code yet`             |
|Most Shared (Overall)|`r message[match(max(likes_count), shares_count)]`|
|Most Shared Post in `r thisMth`            | `r # no code yet`|
|Most Commented  (Overall)|`r message[match(max(comments_count), comments_count)]`|
|                        |`r # if (is.null(fbPosts)) paste(noPosts)`  |
